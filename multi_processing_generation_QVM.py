...
def generate_measurements(packed_arg):
    ...
    results = []
    route_device = cirq_google.Sycamore
    route_device_graph = route_device.metadata.nx_graph
    router = cirq.RouteCQC(route_device_graph)
    ...
    for bit_string,count in zip(labels,tqdm(counts)):
        ...
        translated_ghz_circuit = cirq.optimize_for_target_gateset(
                                c_m, context=cirq.TransformerContext(deep=True), gateset=cirq.SqrtIswapTargetGateset()
                                )
        rep = [count]
        samples = sim_engine.get_sampler(processor_id).run(translated_ghz_circuit, repetitions=rep)
...
if __name__ == '__main__':
    ...
    np.save("results_QVM",conc_results)
    np.save("labels_QVM",labels)
